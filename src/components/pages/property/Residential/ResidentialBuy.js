...
const PropertyCard = ({ property }) => {
  return (
    <Grid item xs={12} sm={6} md={4} lg={4}>
      <RouterLink
        to={`/properties/residential/buy/${property.id}`}
...
const ResidentialBuy = () => {
  ...
  const getData = async () => {
    try {
      ...
      const filterData = response.data.filter(
        (pro) => pro.propertyType === "Residential" && pro.propertyFor === "Buy"
      );
      setData(filterData);
    } catch (error) {
  ...
  const filteredData = data.filter((property) => {
    ...
    if (filters.propertyPlan && property.unitSize !== filters.unitSize) {
      return false;
    }
